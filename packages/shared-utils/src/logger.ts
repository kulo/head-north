...
export interface Logger {
  ...
  errorSafe: (message: string, error: unknown, additionalData?: Record<string, any>) => void;
  warnSafe: (message: string, error: unknown, additionalData?: Record<string, any>) => void;
}
...
const createConsoleLogger = (component?: string): Logger => {
  ...
  return {
    ...
    child: (meta: { component?: string }) => createConsoleLogger(meta.component || component),
    
    // Safe error logging methods
    errorSafe: (message: string, error: unknown, additionalData: Record<string, any> = {}) => {
    ...
    warnSafe: (message: string, error: unknown, additionalData: Record<string, any> = {}) => {
      const errorMessage = getSafeErrorMessage(error);
      const errorStack = getSafeErrorStack(error);
