...
export default class JiraConfig {
  ...
  private _initializeConfig(): JiraConfigType {
    ...
    if (this.useFakeData) {
      ...
      config.fields = {
        epic: getEnvVarWithFallback(
          this.processEnv,
          "JIRA_EPIC_FIELD",
          "customfield_10014",
          "Jira epic field",
        ),
        sprint: getEnvVarWithFallback(
          this.processEnv,
          "JIRA_SPRINT_FIELD",
          "customfield_10020",
          "Jira sprint field",
        ),
        storyPoints: getEnvVarWithFallback(
          this.processEnv,
      ...
      config.fields = {
        epic: getRequiredEnvVar(
          this.processEnv,
          "JIRA_EPIC_FIELD",
          "Jira epic field is required",
        ),
        sprint: getRequiredEnvVar(
          this.processEnv,
          "JIRA_SPRINT_FIELD",
          "Jira sprint field is required",
        ),
        storyPoints: getRequiredEnvVar(
          this.processEnv,
          "JIRA_STORY_POINTS_FIELD",
          "Jira story points field is required",
        ),
      };

      config.connection = {
        user: getRequiredEnvVar(
          this.processEnv,
          "JIRA_USER",
