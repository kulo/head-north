...
describe("Route Consistency", () => {
  ...
  describe("Backend Route Registration", () => {
    it("should be able to import route registry", async () => {
      ...
      const { registerApiRoutes, API_ENDPOINTS: registryEndpoints } =
        await import("../../../apps/backend/src/utils/route-registry");
    ...
    it("should have consistent endpoint definitions", async () => {
      const { API_ENDPOINTS: registryEndpoints } = await import(
        "../../../apps/backend/src/utils/route-registry"
      );

      // Check that all endpoints are defined in both places
      Object.keys(API_ENDPOINTS).forEach((key) => {
        assert.ok(
          registryEndpoints[key],
          `Endpoint ${key} should be defined in route registry`,
        );
        assert.strictEqual(
          registryEndpoints[key],
          API_ENDPOINTS[key],
          `Endpoint ${key} should match between shared-config and route-registry`,
        );
      });
    });
  });
