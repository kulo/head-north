...
export class RoadmapItemParser {
  ...
  constructor(roadmapItems: Record<string, RoadmapItem>, omegaConfig: OmegaConfig) {
    this.roadmapItems = roadmapItems;
    this.omegaConfig = omegaConfig;
    this.validationDictionary = omegaConfig.getValidationDictionary();
  }

  parse(projectId: string, releaseItems: ReleaseItem[] = []): ParsedRoadmapItem {
  ...
  private _updateReleaseItemsExternalState(projectId: string, isRoadmapItemExternal: boolean, hasNoPreReleaseAllowedLabel: boolean, releaseItems: ReleaseItem[]): ReleaseItem[] {
  ...
  private _collectExternal(project: RoadmapItem, releaseItems: ReleaseItem[]): { value: boolean; validations: any[] } {
  ...
  private _collectReleaseInfo(releaseItems: ReleaseItem[]): { isPartOfNarrative: boolean; isAtRisk: boolean } {
    const OR = (a: boolean, b: boolean) => a || b;

    const releaseItemsReleaseNarratives = map( releaseItems, property('isPartOfReleaseNarrative') ) as boolean[];
    const isProjectPartOfNarrative = reduce( releaseItemsReleaseNarratives, OR, false );

    const releaseItemsReleaseRisks = map( releaseItems, property('isReleaseAtRisk') ) as boolean[];
  ...
  private _validateExternalReleaseStages(project: RoadmapItem, releaseItems: ReleaseItem[]): any[] {
