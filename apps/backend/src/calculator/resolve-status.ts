...
export function resolveStatus(issueFields: any, sprint: any, omegaConfig: OmegaConfig): string {
  ...
  if(issueFields.sprint && sprint && sprint.start && new Date(sprint.start) < new Date(issueFields.sprint.startDate)) {
    return jiraConfig?.statusMappings?.[omegaConfig.getItemStatusValues().POSTPONED] || 'postponed'; // POSTPONED_STATUS_ID
  }
