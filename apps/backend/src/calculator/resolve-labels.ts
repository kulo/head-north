...
class LabelResolver {
  ...
  static collectInitiative(project: any, omegaConfig: OmegaConfig) {
    ...
    if(!initiative) {
      const isNonRoadmapTheme = this.parseTheme(project.labels) === 'non-roadmap';
      const validationDictionary = omegaConfig.getValidationDictionary();
      const validations = isNonRoadmapTheme ? [] : [validationDictionary.roadmapItem.missingInitiativeLabel];
      return { value: fallbackInitiative, id: fallbackInitiative, validations };
    }

    const translatedInitiative = translateLabelWithoutFallback('initiative', initiative, omegaConfig);

    if (!translatedInitiative) {
      return {
        value: initiative,
        id: initiative,
        validations: [omegaConfig.getValidationDictionary().roadmapItem.missingInitiativeTranslation(initiative)]
      };
    }
  ...
  static collectTheme(project: any, omegaConfig: OmegaConfig) {
    ...
    const translatedTheme = translateLabelWithoutFallback('theme', theme, omegaConfig);
    if (!translatedTheme) {
      return { value: theme, validations: [omegaConfig.getValidationDictionary().roadmapItem.missingThemeTranslation(theme)] };
    }
  ...
  static collectArea(project: any, omegaConfig: OmegaConfig) {
    ...
    const areas = areaIds.map((area: string) => {
      ...
      if (!translatedArea) {
        return { value: area, validations: [omegaConfig.getValidationDictionary().roadmapItem.missingAreaTranslation(area)] };
      }
