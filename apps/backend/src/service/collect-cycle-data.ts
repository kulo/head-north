 ...
 */
export default async (omegaConfig: OmegaConfig, extraFields: string[] = []): Promise<{
  ...
  releaseItems.forEach((rawReleaseItem: any) => {
    ...
    const matchingIssue = allIssues.flat().find((issue: any) => 
  ...
  if (omegaConfig.isUsingFakeCycleData()) {
    ...
    enhancedAreas = Object.fromEntries(
      Object.entries(enhancedAreasData).map(([id, data]) => [id, { ...data, id }])
    );
  } else {
  ...
  return {
    ...
    assignees,
    areas: enhancedAreas,
    initiatives: Object.entries(initiatives).map(([id, name]) => ({ id, name: name as string })),
    stages,
    teams
  };
};
