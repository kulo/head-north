...
export default class FakeDataGenerator {
  ...
  async getIssuesForSprint(sprintId: string | number, extraFields: string[] = []): Promise<JiraIssue[]> {
    ...
    const currentSprint = this.sprints.find(sprint => sprint.id.toString() === sprintId.toString());
    ...
    roadmapItemKeys.forEach((roadmapItemKey, roadmapItemIndex) => {
      ...
      releaseItemsInThisSprint.forEach((releaseItem, i) => {
        ...
        const issueIndex = roadmapItemIndex * 100 + parseInt(sprintId.toString()) * 10 + i; // Ensure unique IDs
   ...
   * @private
   */
  private _getReleaseItemsForRoadmapItem(roadmapItemKey: string): ReleaseItem[] {
    const stages = ['s0', 's1', 's2', 's3', 's3+']; // Stage progression order
    ...
    for (let i = 0; i < numItems; i++) {
      const assignee = this.assignees[Math.floor(Math.random() * (this.assignees.length - 1)) + 1]; // Skip 'All Assignees'
      const statuses = ['To Do', 'In Progress', 'Done', 'Review'];
      const status = statuses[Math.floor(Math.random() * statuses.length)] || 'To Do';
