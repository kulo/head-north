<template>
  <div class="cycle-overview-container">
    ...
    <a-layout-content id="roadmaps">
      ...
      <div
        v-else-if="!filteredRoadmapData || filteredRoadmapData.length === 0"
        class="no-data"
      >
...
export default {
  ...
  setup() {
    ...
    const filteredRoadmapData = computed(
      () => store.getters.filteredRoadmapData,
    );
    ...
    const activeCycle = computed(() => {
      ...
      const activeCycles = roadmapData.value.cycles.filter(
        (cycle) => cycle.state === "active",
      );
    ...
    onMounted(async () => {
      await Promise.all([fetchRoadmap(), fetchAreas()]);
    });
