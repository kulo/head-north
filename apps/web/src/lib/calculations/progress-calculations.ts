import CycleProgressData from "../models/cycle-progress-data";
...
const recalculateReleaseItemBasedFields =
  (areaTranslation) => (roadmapItem) => {
    const areaIds = roadmapItem.releaseItems
      .map((releaseItem) => releaseItem.areaIds)
      .flat();
    const teams = roadmapItem.releaseItems
      .map((releaseItem) => releaseItem.teams)
      .flat();
    return {
      ...roadmapItem,
...
const filterRoadmapItems =
  (releaseItemPredicate, areaTranslation) => (initiative) => {
...
const createPredicates = (areaMapping, predicate) => {
  const predicates = Object.keys(areaMapping).map((area) => {
    return {
      id: area,
      releaseItemPredicate: createAreaPredicate(area, predicate),
    };
  });
...
const createAreaPredicate = (area, predicate) => {
  return (releaseItem) =>
    releaseItem.areaIds.includes(area) && predicate(releaseItem);
};

export const calculateCycleProgressData = (
  unifiedData,
  releaseItemPredicate = () => true,
  initiativePredicate = () => true,
) => {
