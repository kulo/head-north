...
describe('predicateFilters', () => {
  ...
  describe('createInitiativesPredicate', () => {
    ...
    test('should return true for all items when "All" is selected', () => {
      const predicate = createInitiativesPredicate([{ id: 'all', name: 'All Initiatives' }]);
      expect(predicate(mockReleaseItem)).toBe(true);
    });

    test('should match initiative by initiativeId', () => {
      const predicate = createInitiativesPredicate([{ id: 1, name: 'Test Initiative' }]);
      expect(predicate(mockReleaseItem)).toBe(true);
    });

    test('should return false for non-matching initiative', () => {
      const predicate = createInitiativesPredicate([{ id: 2, name: 'Other Initiative' }]);
      expect(predicate(mockReleaseItem)).toBe(false);
    });
  ...
  describe('createStagesPredicate', () => {
    ...
    test('should return true for all items when "All" is selected', () => {
      const predicate = createStagesPredicate([{ id: 'all', name: 'All Stages' }]);
      expect(predicate(mockReleaseItem)).toBe(true);
    });

    test('should match stage by stage property', () => {
      const predicate = createStagesPredicate([{ value: 's1', name: 'Stage 1' }]);
      expect(predicate(mockReleaseItem)).toBe(true);
    });

    test('should return false for non-matching stage', () => {
      const predicate = createStagesPredicate([{ value: 's2', name: 'Stage 2' }]);
      expect(predicate(mockReleaseItem)).toBe(false);
    });
  });
  ...
  describe('createCyclePredicate', () => {
    ...
    test('should return false for all items and log error when no cycle is selected', () => {
      ...
      expect(predicate(mockReleaseItem)).toBe(false);
      expect(consoleErrorSpy).toHaveBeenCalledWith('createCyclePredicate: No cycle provided. Client code must ensure a valid cycle is passed.');
    });

    test('should return false for all items and log error when cycle is "all"', () => {
      const predicate = createCyclePredicate('all');
      expect(predicate(mockReleaseItem)).toBe(false);
      expect(consoleErrorSpy).toHaveBeenCalledWith('createCyclePredicate: Invalid cycle ID provided. Client code must ensure a valid cycle ID is passed.');
    });
