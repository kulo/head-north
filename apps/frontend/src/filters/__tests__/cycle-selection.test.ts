...
describe('Cycle Selection Logic', () => {
  ...
  describe('selectBestCycle function behavior', () => {
    ...
    test('should select oldest future cycle when no active cycles', async () => {
      const futureOnlyCycles = mockCycles.filter(cycle => 
        cycle.state === 'planned' || cycle.state === 'closed' || cycle.state === 'completed'
      );
    ...
    test('should select oldest closed cycle when no active or future cycles', async () => {
      const closedOnlyCycles = mockCycles.filter(cycle => 
    ...
    test('should handle cycles with missing dates gracefully', async () => {
      ...
      mockCycleDataService.getAllCycles.mockResolvedValue(cyclesWithMissingDates);
  ...
  describe('_ensureSelectedCycle action', () => {
    test('should not change cycle if one is already selected', async () => {
      const existingCycle = { id: 'existing', name: 'Existing Cycle', state: 'active' };
    ...
    test('should select best cycle when none is selected', async () => {
      const cycles = [
        { id: 'cycle1', name: 'Active Cycle', state: 'active', start: '2024-01-01' },
        { id: 'cycle2', name: 'Future Cycle', state: 'planned', start: '2024-06-01' }
      ];
    ...
    test('should handle error when fetching cycles fails', async () => {
      mockCycleDataService.getAllCycles.mockRejectedValue(new Error('Network error'));
